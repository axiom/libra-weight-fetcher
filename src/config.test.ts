...
describe("config.ts", () => {
  test("targetWeightConfig has reasonable values", () => {
    expect(targetWeightConfig.startWeight).toBeGreaterThan(
  ...
  test("dates are valid ISO strings", () => {
    expect(new Date(targetWeightConfig.startDate).toString()).not.toBe(
      "Invalid Date",
    );
    expect(new Date(targetWeightConfig.targetDate).toString()).not.toBe(
      "Invalid Date",
    );
  });
});
